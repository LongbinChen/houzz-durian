...
class PipeParser:
    ...
    def _validate_pipe(self):
      ...
      self.job_keys = [k for k in self.pipe['jobs'].keys() if 'module' in self.pipe["jobs"][k] and 'template' not in self.pipe["jobs"][k]] 
    ...
    def _update_job_hash(self, job, job_hash):
        ...
        for lj in latching_job:
          self.info("job %d is latched to with id %d, job_name:%s, status: %s " % (job.id, lj.id, lj.job_name,lj.status))
        ...
        for k, v in self.pipe["jobs"][i]['input'].items():
          ...
          for va in v:
            if va in self.output_map:
              dp = JobDependency(job_name_src = durianutils.get_full_job_name(self.pipe_name, self.output_map[va]), 
    ...
    def _load_pipe_in_json(self, pipe):
      ...
      usr = User.objects.get(username='durian')
      grp = Group.objects.get(name="durian")
      self.pp = Pipe(owner_id = usr, group_id = grp, pipe_name = self.pipe_name, 
                     pipe_def = json.dumps(pipe), status = "created")
      self.pp.save()
      ...
      self.pp.save()
      self.info("New Pipe is created  with %d. you can use command 'dr runpipe %d' to start running the pipe" % (self.pp.id, self.pp.id))
    ...
    def _lookup_input_data(self, job_conf, module_conf, input_name):
      ...
      for inn in input_name_conf:
        if self.is_data_file.get(inn, False)  or is_data_by_config: # check if it is data by config or by smart search
