    ...
    def _update_job_status(self, job, status):
       ...
       for j in all_latched_jobs:
          ...
          if (status == Job.COMPLETED):
            #self.info("remove dependency on job job_name_src %s " % j.job_name)
    ...
    def _run_command(self, cmd):
        ...
        try:
          ...
          if (text != 0):
              ...
              self.info("\n".join(open(os.path.join(self.job_dir, "stderr_%d.txt" % self._cmd_count), "r").readlines()))
              self._fail_job()
    ...
    def _link_file_from_storage(self, file_hash, data_name): 
        if local_storage:
            ...
            if not os.path.exists(storage_location):
               print "============="
               print "[ERROR]"
               print "It seems the input file '%s', which is linked to %s, doesn't exist. Please make sure the dependent jobs are completed" % (data_name, storage_location)
               self._fail_job()
        ...
        if (run_local):
            ...
            for f in flds:
                if "input" in self.module and f in self.job_conf["input"]:
                    ...
                    trans_flds.append(str(self.job_conf.get("parameters", {}).get(f, self.module["parameters"][f].get('default', None))))
                else:
                    trans_flds.append(f)
